...
export async function sendSms(phone: string, code: string) {
  ...
  const accessKeyId = 'LTAIzeVTN3gAktzM';
  const secretAccessKey = 'xyUxue1Qhu9XPBjQPeXFzOcPpcVTxQ';
  //初始化sms_client
  ...
  try {
    let res = await smsClient.sendSMS({
      ...
      SignName: '萤火乐收', // 必填: 短信签名 - 可在短信控制台中找到
      TemplateCode: 'SMS_137422292', // 必填: 短信模板 - 可在短信控制台中找到，发送国际 / 港澳台消息时，请使用国际 / 港澳台短信模版
